...
def holobalance_disorientation(spark, client):
    ...
    dfDisorientation_weekly_sum = dfDisorientation.groupBy('Patient_id', 'Week', 'entry_resource_code_coding_display').agg(sum(col("entry_resource_component_valueBoolean").cast("double")).alias("Sum"))
    dfDisorientation_weekly_sum = dfDisorientation_weekly_sum.orderBy("Week")
    ...
    url = 'https://https:bda-api:8100/homepage/holobalance_disorientation'
...
def main(args):
    ...
    try:
      ...
      holobalance_disorientation(spark, hdfs)
